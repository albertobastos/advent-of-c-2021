...
void count_paths0(Cave* c, int caves_count, int* visited, int* paths_count) {
  ...
  for (i=0; c->links[i] != NULL; i++) {
    ...
    count_paths0(c->links[i], caves_count, visited_copy, paths_count);
  }
}
...
void count_paths(Cave* start, int caves_count, int* paths_count) {
  ...
  count_paths0(start, caves_count, visited, paths_count);
}
