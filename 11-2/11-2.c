...
int maybe_restore(int* grid, int x, int y) {
  int i = idx(x,y);
  if (grid[i] == FLASHED) {
    grid[i] = 0;
    return 1;
...
int do_step(int* grid) {
  ...
  do {
    ...
    for (x=0; x<ROWS; x++) {
      for (y=0; y<COLS; y++) {
        any_flash |= maybe_flash(grid, x, y);
      }
    }
  } while(any_flash);
  ...
  for (x=0; x<ROWS; x++) {
    for (y=0; y<COLS; y++) {
      all_flashed &= maybe_restore(grid, x, y);
    }
  }
  return all_flashed;
}