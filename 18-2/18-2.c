...
SN** read_file(FILE* in, int* count) {
  ssize_t len = 0;
  char* buf;
...
SN* clone(SN* n) {
  if (n == NULL) return n;
  SN* cpy = malloc(sizeof(SN));
  cpy->regular = n->regular;
  if (cpy->regular == PAIR) {
    cpy->left = clone(n->left);
    cpy->right = clone(n->right);
    cpy->left->parent = cpy;
...
int main(int argc, char** args) {
  ...
  //args[1] = "18-2/sample.in";
  if ((fp = fopen(args[1], "r")) == NULL) {
    printf("Cannot open file: %s\n", args[1]);
    exit(1);
  }

  sns = read_file(fp, &count);
  ...
  for (int i=0; i<count; i++) {
    for (int j=0; j<count; j++) {
      if (i != j) {
        mag = magnitude(add(clone(sns[i]), clone(sns[j])));
        if (mag > max) max = mag;
      }
    }
  }
