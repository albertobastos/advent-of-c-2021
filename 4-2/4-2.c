...
int check_filled(Card*);
int get_item(Card*, int, int);
int card_score(Card*, int);
CardNode *remove_filled(CardNode*);
...
int main(int argc, char** args) {
  ...
  last_filled = NULL;
  for (i=0;i<NUMBERS && last_filled == NULL;i++) {
    last_filled = punch_cards(first_card, numbers[i]);
    first_card = remove_filled(first_card);
  }
  ...
  printf("Answer: %d\n", card_score(last_filled, numbers[i-1]));
...
Card *punch_cards(CardNode* list, int val) {
  ...
  if (first->next == NULL && check_filled(first->card))
    return first->card;

  return NULL;
}

int check_filled(Card* card) {
  int row, col;
...
int card_score(Card* card, int lastCalled) {
  int sum = 0;
  int i;
  for (i=0;i<NCOLS*NROWS;i++)
...
CardNode *remove_filled(CardNode* first) {
  CardNode *new_first = NULL;
  CardNode *prev = NULL;
  CardNode *tmp;
  while(first != NULL) {
    if (check_filled(first->card)) {
      if (prev != NULL) prev->next = first->next;
      tmp = first;
      first = first->next;
    } else {
      if (new_first == NULL) new_first = first;
      prev = first;
      first = first->next;
    }
  }
